...
import { toArray, isString, isPlainObject, isArray, keys, extend, tryRootRequire } from 'chek';
...
export function copy(src: string, dest: string): boolean {
    ...
    try {
        ...
        log.warn(`failed to copy ${getRelative(parsedSrc || 'undefined')} to ${getRelative(parsedDest || 'undefined')}.`);
        return false;
    }
...
export function copyAll(copies: CopyTuple[] | CopyTuple | IMap<ICopy> | string[]) {
    ...
    function logResults() {
        ...
        if (success || failed) {
            if (failed > success) {
                if (!success)
                    log.error(`${failed} copies failed to processes with 0 succeeding.`);
                else
                    log.warn(`${failed} copies failed to processes with ${success} succeeding.`);
            }
            else {
                log.info(`${success} items ${'successfully'} copied with ${failed} copies failing.`);
            }
        }
    }
